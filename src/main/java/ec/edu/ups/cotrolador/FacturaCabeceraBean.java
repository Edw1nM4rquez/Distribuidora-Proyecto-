...
public class FacturaCabeceraBean implements Serializable {
    ...
    public boolean validarDetalle(Stock stock){
        for (Stock stock1: stockList){
            if (stock.getCodigo()==stock1.getCodigo()){
                return true;
            }
        }
        return false;
    ...
    public void calcularValores(){
        ...
        facturaCabecera.setTotal(round(facturaCabecera.getSubtotal()+facturaCabecera.getIva(),2));
    }
    ...
    public String visualizarFactura(FacturaCabecera facVisualizar){
        this.facturaVisualizar=ejbFacturaCabeceraFacade.find(facVisualizar.getNumero());
